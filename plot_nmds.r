...
nmds_analyze <- function(abundance, group){
  ...
  Data <- read.table(abundance,sep = '\t', row.names= 1,head = TRUE, check.names = FALSE, quote = "")
  groups <- read.delim(group, header = T, sep = "\t", row.names = 1, check.names = F, stringsAsFactors = F)
  data <- apply(Data,2,as.numeric)
  row.names(data) <- row.names(Data)
  ...
  a <- list(sample=sample_site, nmds=nmds1)
  return(a)
}
...
plot_nmds <- function(abundance, group, prefix){
  ...
  p <- ggplot(sample_site,aes(NMDS1, NMDS2)) +
    geom_point(aes(color = group , shape = group), size = 3, alpha = 0.8) + #修改点的透明度、大小
    ...
    scale_x_continuous(limits = c(x_n,x_m))+
    scale_y_continuous(limits = c(y_n,y_m))+ #设置坐标轴范围
